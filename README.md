...
7. **Переустановка зависимостей проекта** — для этого нужно:
   - удалить папку node_modules и файл package-lock.json, используя команду `rm -rf node_modules package-lock.json`. Можент возникнуть ошибка с символом -rf, для этого используйте другую команду `rm package-lock.json`. Файл package-lock.json удалиться.
